...
def cifar10_noniid(args, dataset, num_users, n_list, k_list):
    ...
    # sort labels
    idxs_labels = np.vstack((idxs, labels))# ndarray (2,50000), the first row is sample ids, the second row is the class ids of samples
    idxs_labels = idxs_labels[:, idxs_labels[1, :].argsort()]# Sort by class ID
    idxs = idxs_labels[0, :]
    label_begin = {}# Starting index of each class. For example, if the sample labels are [0, 0, 0, 1, 2], then label_begin = {0: 0, 1: 3, 2: 4}.
...
def eurosat_noniid(args, dataset, y,num_users, n_list, k_list):
    """
    Sample non-I.I.D client data from MNIST dataset
    :param dataset:
    :param num_users:
    ...
    for j in idxs_labels[1, :]:
        if j not in label_num:
            label_num[j]=1
        else:
            label_num[j]+=1
    min_label= min(label_num, key=lambda k: label_num[k])
...
def eurosat_noniid_lt(args,test_dataset, y,num_users, classes_list):
    """
    Sample non-I.I.D client data from MNIST dataset
    :param dataset:
    :param num_users:
    ...
    for j in idxs_labels[1, :]:
        if j not in label_num:
            ...
            label_num[j]+=1
    min_label= min(label_num, key=lambda k: label_num[k])
    print('min_test_class_num:{}'.format(label_num[min_label]))
    for i in range(num_users):
        k = args.test_shots
        classes = classes_list[i]
        print("local test classes:", classes)
...
def tiny_noniid(args, dataset, num_users, n_list, k_list):
    ...
    for j in idxs_labels[1, :]:
        if j not in label_num:
            ...
            label_num[j] += 1
    min_label = min(label_num, key=lambda k: label_num[k])
    print('min_train_class_num:{}'.format(label_num[min_label]))
...
def tiny_noniid_lt(args,test_dataset, num_users, classes_list):
    """
    Sample non-I.I.D client data from MNIST dataset
    :param dataset:
    :param num_users:
...
def fashion_noniid(args, dataset, num_users, n_list, k_list):
    """
    Sample non-I.I.D client data from MNIST dataset
    :param dataset:
    :param num_users:
...
def fashion_noniid_lt(args, test_dataset, num_users, n_list, k_list, classes_list):
    """
    Sample non-I.I.D client data from MNIST dataset
    :param dataset:
    :param num_users:
