...
在联邦学习的背景下，想要通过 GAN 来生成 中间特征/图片，存在 训练过程中的模式坍塌问题。回归合成虚拟样本的目标，模拟其他客户端上的数据，帮助其他客户端的模型进行调整。判别器：使用指定客户端上的 high-level encoder, classifier 的输出 作为学习的标准，如果合成出来的样本通过 目标的 分类器，输出结果 和 投入真实样本的结果 较为 一致，那么就可以 认为合成样本 和 真实样本时较为一致的，然后使用合成样本 对客户端上的数据进行补充。 

----

通过@run_stage1.py 完成了各个客户端在一阶段的训练，得到了不错的平均准确率。后面我想要学习FedGen的做法，通过将已经训练过的本地的模型作为判别器。对于每一个类别，选取几个该类别较为优质的本地模型，将合成样本在上面跑一下，看一下运行结果的效果如何（如果合成的是低级特征，就看看通过判断 运行后的 高级特征效果，和 后续分类器的效果）

---

帮我详细分析一下将Generator引入多阶段FedMPS训练框架的思路，给出详细的规划
